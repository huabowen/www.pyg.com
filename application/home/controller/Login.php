...
class Login extends Controller
{
    ...
    public function alicallback()
    {
        ...
        //实现自动注册登录功能
        $user = \app\common\model\User::where('open_type', 'alipay')->where('openid', $info['user_id'])->find();
        if($user){
            $user->nickname = $info['nick_name'] ?? $info['user_id'];
            $user->save();
        }else{
            \app\common\model\User::create([
                'nickname' => $info['nick_name'] ?? $info['user_id'],
                'open_type' => 'alipay',
                'openid' => $info['user_id']
            ]);
        }
